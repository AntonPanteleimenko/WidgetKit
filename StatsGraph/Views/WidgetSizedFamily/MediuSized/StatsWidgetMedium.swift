...
struct StatsWidgetMedium: View {
    ...
    var body: some View {
        Link(destination: URL(string: "https://covid19.gov.ua/en/")!) {
    ...
    mutating fileprivate func setWidgetFamilyValues(for widgetFamily: WidgetFamily) {
        switch widgetFamily {
        case .systemMedium:
            self.spacing = CGFloat(11)
            self.fontTitle = Font.custom("Futura-Medium", size: 17)
            self.fontDate = Font.custom("Futura-Medium", size: 12)
            self.fontUnits = Font.custom("Futura-Medium", size: 14)
            self.fontCategory = Font.custom("Futura-Medium", size: 12)
        default:
            self.spacing = CGFloat(15)
            self.fontTitle = Font.custom("Futura-Medium", size: 15)
            self.fontDate = Font.custom("Futura-Medium", size: 8)
            self.fontUnits = Font.custom("Futura-Medium", size: 10)
            self.fontCategory = Font.custom("Futura-Medium", size: 8)
        }
    }
}
...
struct StatsWidgetMedium_Previews: PreviewProvider {
    static var previews: some View {
        StatsWidgetMedium(data: Model.stub)
